...
Filen pris_historikk.db opprettes automatisk ved første kjøring.
"""
    ...
    with get_conn() as conn:
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS prices (
                ...
                url TEXT,
                status TEXT DEFAULT 'ok',  -- ok, missing_url, scrape_error, blocked
                error_msg TEXT,
...
def save_price(product_id: str, competitor: str, price: float | None,
    ...
    """Lagrer en pris-observasjon. Erstatter dagens hvis den allerede finnes."""
...
def get_latest_prices() -> list[dict]:
    """Hent alle priser for nyeste dato."""
    with get_conn() as conn:
...
def get_history(product_id: str, days: int = 30) -> list[dict]:
    """Henter historikk for ett produkt, siste N dager."""
    with get_conn() as conn:
        rows = conn.execute("""
